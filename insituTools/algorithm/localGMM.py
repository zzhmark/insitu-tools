...
def local_gmm(global_label, levels, n=10, image=None):
    assert type(image) is np.ndarray and image.dtype == np.uint8 and len(image.shape) == 2 or image is None, \
        'The input image has to be a uint8 2D numpy array, or omitted.'
    assert type(global_label) is np.ndarray and global_label.dtype == np.uint8 and len(global_label.shape) == 2, \
        'The input global_label has to be a uint8 2D numpy array.'
    assert len(levels) == np.max(global_label), \
